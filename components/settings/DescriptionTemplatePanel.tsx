...
import { EtsyDescriptionTemplate } from '../../types';
import Button from '../common/Button';
import TextArea from '../common/TextArea';
import { downloadJson, readJsonFromFile } from '../../utils/fileUtils';
import Spinner from '../common/Spinner';

const DescriptionTemplatePanel: React.FC = () => {
    const { templates, addTemplate, updateTemplate, deleteTemplate } = useTemplates<EtsyDescriptionTemplate>('ETSY_DESCRIPTION_TEMPLATES');
    const [editing, setEditing] = useState<Partial<EtsyDescriptionTemplate>>({});
    const [name, setName] = useState('');
    const [content, setContent] = useState('');
    const [isImporting, setIsImporting] = useState(false);

    const handleSelectForEdit = (template: EtsyDescriptionTemplate) => {
        setEditing(template);
    ...
    const handleExport = () => {
        ...
        downloadJson(exportData, 'etsy-descriptions.json');
    };
    ...
    const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setIsImporting(true);
        try {
            const importedData = await readJsonFromFile<Omit<EtsyDescriptionTemplate, 'id' | 'createdAt'>[]>(file);
            if (!Array.isArray(importedData)) {
                throw new Error('Invalid format: JSON file should contain an array.');
            }
            
            if (window.confirm(`This will import ${importedData.length} new description template(s). Continue?`)) {
    ...
    return (
        <div>
            <h3 className="text-xl font-bold text-white mb-4">Etsy Description Templates</h3>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col gap-4" style={{maxHeight:400 }}>
                    ...
                    <div>
                        <label className="text-sm text-gray-400 mb-1 block">Template Content</label>
                        <TextArea
                            value={content}
                            onChange={e => setContent(e.target.value)}
                            className="h-40"
                            placeholder="Enter the full Etsy description here..."
                        />
                    </div>
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex items-center gap-2">
                            <Button variant="ghost" className="!text-xs !px-2 !py-1" onClick={() => document.getElementById('desc-template-import-input')?.click()} disabled={isImporting}>
                            ...
                            <input type="file" id="desc-template-import-input" accept=".json" className="hidden" onChange={handleImport} />
                        </div>
                    </div>
                            ...
                            <div key={template.id} className="bg-black/20 p-3 rounded-lg">
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400 truncate max-w-xs">{template.content.split('\n')[0]}</p>
                                    </div>
